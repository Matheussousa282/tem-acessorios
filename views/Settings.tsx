    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="mt-6">
        {activeTab === 'permissions' && localPerms && (
           <div className="max-w-4xl space-y-8 bg-white dark:bg-slate-900 p-10 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-sm animate-in slide-in-from-left-4">
              ...
              <button onClick={handleSavePermissions} disabled={isSaving} className="w-full h-16 bg-primary text-white rounded-2xl font-black text-xs uppercase shadow-xl flex items-center justify-center gap-2">
        ...
        {activeTab === 'general' && (
           <div className="max-w-4xl bg-white dark:bg-slate-900 p-10 rounded-[3rem] border shadow-sm">
              <div className="flex items-center gap-10">
                 <div className="size-32 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-300 flex items-center justify-center overflow-hidden">
                 ...
                 <div className="flex-1 space-y-4">
                    ...
                    <input type="text" value={localConfig.companyName} onChange={e => setLocalConfig({...localConfig, companyName: e.target.value})} className="w-full h-14 bg-slate-50 dark:bg-slate-800 rounded-2xl px-6 font-black uppercase text-sm border-none outline-none focus:ring-2 focus:ring-primary" />
                    <button onClick={() => logoInputRef.current?.click()} className="text-[10px] font-black text-primary uppercase underline">Trocar Logotipo</button>
        ...
        {activeTab === 'db' && isAdmin && (
           <div className="max-w-4xl bg-slate-900 p-12 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
              ...
              <p className="text-slate-400 mt-4 uppercase text-xs">Força a sincronização das tabelas e colunas do banco de dados.</p>
...
const getIconForModule = (key: string) => {
  const icons: Record<string, string> = {
    dashboard: 'dashboard', pdv: 'point_of_sale', cashControl: 'account_balance_wallet', customers: 'groups', reports: 'monitoring', inventory: 'inventory_2', balance: 'inventory', incomes: 'payments', expenses: 'money_off', financial: 'account_balance', settings: 'settings', serviceOrders: 'build', cardManagement: 'credit_card'
  };
...
const TabButton: React.FC<{ active: boolean; onClick: () => void; icon: string; label: string }> = ({ active, onClick, icon, label }) => (
  <button onClick={onClick} className={`flex items-center gap-2 px-6 py-4 border-b-2 transition-all whitespace-nowrap ${active ? 'border-primary text-primary font-black' : 'border-transparent text-slate-500'}`}>
