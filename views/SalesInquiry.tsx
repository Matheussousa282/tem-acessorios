    ...
    <div className="min-h-screen bg-slate-100 dark:bg-background-dark font-sans text-[11px] uppercase font-bold">
         ...
         {viewingDetail && (
           <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 border p-4">
                 <div><p className="font-black text-[8px] uppercase text-slate-500">Identificação do Cliente:</p><p className="font-bold">{viewingDetail.client || 'CONSUMIDOR FINAL'}</p></div>
                 <div><p className="font-black text-[8px] uppercase text-slate-500">Operador / Vendedor:</p><p className="font-bold">{getUserData(viewingDetail.vendorId)?.name || 'BALCÃO'}</p></div>
              </div>
              <table className="w-full mt-4 text-left border-collapse">
                 ...
                 <tfoot><tr className="font-black text-lg"><td colSpan={3} className="pt-4 text-right uppercase tracking-widest">Valor Total:</td><td className="pt-4 text-right">R$ {viewingDetail.value.toLocaleString('pt-BR')}</td></tr></tfoot>
              </table>
           </div>
         )}
      </div>
      ...
      <div className="print:hidden h-full flex flex-col">
        <header className="bg-primary p-4 flex items-center justify-between text-white shadow-lg shrink-0">
          ...
          <div className="flex items-center gap-2"><span className="size-2 bg-emerald-400 rounded-full animate-pulse"></span><span className="text-[10px] font-black uppercase tracking-widest">Banco Sincronizado</span></div>
        </header>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {sales.map(s => (
                <tr key={s.id} onClick={() => setViewingDetail(s)} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 cursor-pointer border-b transition-colors">
                  ...
                  <td className="px-6 py-3 text-right font-black tabular-nums">R$ {s.value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {viewingDetail && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in print:hidden">
           <div className="bg-white dark:bg-slate-900 w-full max-w-5xl h-[85vh] rounded-[2rem] shadow-2xl flex flex-col overflow-hidden text-slate-700">
              <div className="p-4 border-b flex justify-between items-center shadow-sm bg-slate-50 dark:bg-slate-800/50">
              ...
              <div className="p-8 space-y-6 overflow-y-auto custom-scrollbar">
                 <div className="grid grid-cols-3 gap-6">
                    <div className="space-y-1"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Loja / Unidade</p><p className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg text-xs font-black uppercase border">{viewingDetail.store}</p></div>
                    <div className="space-y-1"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Vendedor Balcão</p><p className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg text-xs font-black uppercase border">{getUserData(viewingDetail.vendorId)?.name || 'NÃO INFORMADO'}</p></div>
                 ...
                 <div className="border dark:border-slate-800 rounded-2xl overflow-hidden shadow-inner">
                    <table className="w-full text-left text-[10px] border-collapse">
                       <thead className="bg-slate-50 dark:bg-slate-800/80 border-b"><tr><th className="px-4 py-3 uppercase tracking-widest font-black">Ref SKU</th><th className="px-4 py-3 uppercase tracking-widest font-black">Produto</th><th className="px-4 py-3 text-right uppercase tracking-widest font-black">Qtd</th><th className="px-4 py-3 text-right uppercase tracking-widest font-black">Vr. Total</th></tr></thead>
                       <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                          {viewingDetail.items?.map((item, i) => (
                            <tr key={i} className="hover:bg-slate-50 dark:hover:bg-slate-800/40"><td className="px-4 py-3 font-mono text-slate-400">{item.sku}</td><td className="px-4 py-3 uppercase font-bold">{item.name}</td><td className="px-4 py-3 text-right font-black">{item.quantity}</td><td className="px-4 py-3 text-right font-black text-primary">R$ {(item.quantity * item.salePrice).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td></tr>
                          ))}
                       </tbody>
                    </table>
                 </div>
