...
const CashMovement: React.FC = () => {
  ...
  const sessionData = useMemo(() => {
    ...
    return { allRecords, saldoAnterior, totalEntradas, totalSaidas, saldoFinal };
  ...
  if (viewingSession) {
    return (
      <div className="p-6 space-y-6 animate-in slide-in-from-right-10 duration-500 pb-20">
        {/* HEADER DETALHE */}
        <div className="flex justify-between items-start bg-slate-50 dark:bg-slate-900/40 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
        ...
        {/* CORPO DO DETALHE */}
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm flex flex-col min-h-[600px]">
           {/* SUB-MENU TABS */}
